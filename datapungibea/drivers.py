...
class getDatasetlist():
    def __init__(self,query):
        self._query = query  #TODO: load the connection data here if empty
...
class getNIPA():
    def __init__(self,baseRequest={},connectionParameters={},userSettings={}):
        ...
        if baseRequest =={}:
           ...
           self._baseRequest  = self.__connectInfo.baseRequest
        else:
           self._baseRequest = baseRequest #TODO: 
    ...
    def NIPA(self,
        ...
        payloadValues = self._baseRequest
        payloadValues['params'].update({'TABLENAME': tableName})
        payloadValues['params'].update(payload)
        
        # TODO: try loading different frenquencies if no return
        #
        nipa = requests.get(**payloadValues)
